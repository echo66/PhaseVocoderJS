function PhaseVocoder(winSize, sampleRate) {
	...
	this.STFT_drom = function(inputFrame, windowFrame, wantedSize, out) {
		...
		var heap = fourier.custom.alloc(winSize, 3);

		// Create instance of FFT runner
		var fft = fourier.custom["fft_f64_"+winSize+"_asm"](stdlib, null, heap);

		// Init twiddle factors
		fft.init();

		fourier.custom.array2heap(_inputFrame, new Float64Array(heap), winSize, 0);
		fourier.custom.array2heap(new Float64Array(winSize), new Float64Array(heap), winSize, winSize);

		// Run transformations
		fft.transform();

		fourier.custom.heap2array(new Float64Array(heap), out.real, wantedSize, 0);
		fourier.custom.heap2array(new Float64Array(heap), out.imag, wantedSize, winSize);
	...
	this.STFT_jens = function(inputFrame, windowFrame, wantedSize, out) {
		var winSize = windowFrame.length;
		var _inputFrame = new Array(winSize);
	...
	this.ISTFT_drom = function(real, imag, windowFrame, restoreEnergy, timeFrame) {
		...
		var fft = fourier.custom["fft_f64_"+size+"_asm"](stdlib, null, heap);
		fft.init();
		fourier.custom.array2heap(imag, new Float64Array(heap), size, 0);
		fourier.custom.array2heap(real, new Float64Array(heap), size, size);
		fft.transform();
		fourier.custom.heap2array(new Float64Array(heap), timeFrame, size, size);
	...
	this.ISTFT_jens = function(real, imag, windowFrame, restoreEnergy, output2) {
